...
MCP는 결국 AI와 진단 도구를 잇는 표준 단자입니다. Ghidra(바이너리), Burp(웹 트래픽), jadx(APK)를 각각 플러그인과 브리지로 Claude에 물리면 복붙 없이 AI가 직접 도구를 부려서 분석합니다. 핵심은 localhost 기본에 승인·스코프 설정을 더해 안전하게 쓰는 것입니다.
